...
public class HorizontalProgressWheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int mLargeCircleRadius = 11;
        int mSmallCircleRadius = 6;

        int circlesCount = mCanvasClipBounds.width() / (mProgressLineWidth + mProgressLineMargin);
        ...
        for (int i = 0; i < circlesCount; i++) {
            if (i < (circlesCount / 4)) {
                mProgressLinePaint.setAlpha((int) (255 * (i / (float) (circlesCount / 4))));
            } else if (i > (circlesCount * 3 / 4)) {
                mProgressLinePaint.setAlpha((int) (255 * ((circlesCount - i) / (float) (circlesCount / 4))));
            } else {
                mProgressLinePaint.setAlpha(255);
            }

            // Calculate the center position of the current circle
            float centerX = -deltaX + mCanvasClipBounds.left + i * (mProgressLineWidth + mProgressLineMargin);
            float centerY = mCanvasClipBounds.centerY();

            // Draw the large filled circle
            if (i % 5 == 0) {
                canvas.drawCircle(centerX, centerY, mLargeCircleRadius, mProgressLinePaint);
            }
            // Draw the small filled circles
            else {
                canvas.drawCircle(centerX, centerY, mSmallCircleRadius, mProgressLinePaint);
            }
        }

        // Draw the center black line
        mProgressLinePaint.setColor(Color.BLACK);
        mProgressLinePaint.setStrokeCap(Paint.Cap.ROUND);

        canvas.drawLine(
                mCanvasClipBounds.centerX(),
                mCanvasClipBounds.centerY() - mProgressLineHeight,  // Modify the y-coordinate
                mCanvasClipBounds.centerX(),
                mCanvasClipBounds.centerY() - (mLargeCircleRadius * 2.0f),  // Modify the y-coordinate
                mProgressLinePaint);
    }
